...
RUN mvn package
RUN mvn sonar:sonar -Pcoverage -Dsonar.host.url=${SONARQUBE_URL} -Dsonar.login=${SONARQUBE_TOKEN}
 #RUN if [ "$SONARQUBE_ENV" = "prod" ] ; then mvn sonar:sonar -Pcoverage -Dsonar.host.url=${SONARQUBE_URL} -Dsonar.login=${SONARQUBE_TOKEN}; else echo "${ENVIRONMENT} no envia a sonar"; fi
ENV SONARQUBE_TOKEN=''
ENV SONARQUBE_URL=''
